...
const Time = require('../TestData/time_utils');
...
const SINGLE_CYCLE = process.env.SINGLE_CYCLE === 'true';
...
test('24/7 Skylink Sensor Testing - Continuous Operation', async ({ page }) => {
  ...
  while (true) {
    try {
      ...
      if (SINGLE_CYCLE) {
        console.log('✅ SINGLE_CYCLE mode enabled - exiting after one cycle.');
        break;
      }
  ...
  try {
    switch (effectiveSlot.location) {
      ...
      case 'Dining room':
        ...
        responseStatus = response.status();
        console.log('✅ Dining room PIR Sensor Test - Response Status:', responseStatus);
        break;
